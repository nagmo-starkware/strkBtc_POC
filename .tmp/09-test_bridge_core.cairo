...
mod tests {
    ...
    // Interface for commit 9 - adds deposit_request
    #[starknet::interface]
    ...
    fn test_btc_whitelist() {
        ...
        let token: ContractAddress = 'token'.try_into().unwrap();
        let registry: ContractAddress = 'registry'.try_into().unwrap();
        let btc_address: felt252 = 'bc1qxy2kgdygjrsqtzq2n0yrf2493p83kkfjhx0wlh';
    ...
    // NOTE: test_deposit_request_already_processed_fails requires the mint to succeed
    // (threshold reached), which needs the real token contract. This is an integration
    // test better suited for a separate test suite with full contract deployment.
}
